...
<mark>两种类型要么相同，要么不同</mark>。<mark>[Named type](https://go.dev/ref/spec#Types)(预先声明的类型、定义的类型和类型参数称为命名类型。如果别名声明中给出的类型是命名类型，则别名表示命名类型。) 总是不同于其他任何类型。</mark>

否则，如果两个类型的底层类型字面量结构相同，也即<mark>它们具有相同的字面结构并且相应组件也具有相同的类型</mark>，那么这两个类型就是相同的。详细说明：
