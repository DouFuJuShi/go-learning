...
transactionTimeOut=6000 # 指定TM(Producer)在20秒内应将最终确认状态发送给TC(Broker)，否则引发消息回查。默认为6秒。
transactionCheckMax=15 # 指定最多回查5次，超过后将丢弃消息并记录错误日志。默认15次。
transactionCheckInterval=30000 # 指定设置的多次消息回查的时间间隔为30秒。默认为30秒
